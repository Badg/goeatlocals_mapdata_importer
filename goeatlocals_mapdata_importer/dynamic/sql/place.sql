...
ALTER TABLE __use_schema__.osm_city_point ALTER COLUMN place TYPE city_place USING place::city_place;
...
BEGIN;
  UPDATE __use_schema__.osm_continent_point
...
ALTER TABLE __use_schema__.osm_country_point DROP CONSTRAINT IF EXISTS osm_country_point_rank_constraint;

-- etldoc: ne_10m_admin_0_countries   -> osm_country_point
-- etldoc: osm_country_point          -> osm_country_point

CREATE OR REPLACE FUNCTION __use_schema__.update_osm_country_point()
RETURNS VOID AS $$
BEGIN

  UPDATE __use_schema__.osm_country_point AS osm
  SET
    "rank" = 7,
  ...
  WITH important_country_point AS (
      ...
      FROM mapdata_prod.ne_10m_admin_0_countries AS ne, __use_schema__.osm_country_point AS osm
      WHERE
...
--       ST_Distance(osm.geometry, ne.geometry) AS distance,
    ...
    FROM __use_schema__.osm_country_point osm,
  ...
  UPDATE __use_schema__.osm_country_point
  SET tags = update_tags(tags, geometry)
  WHERE COALESCE(tags->'name:latin', tags->'name:nonlatin', tags->'name_int') IS NULL;
...
SELECT __use_schema__.update_osm_country_point();

-- ALTER TABLE osm_country_point ADD CONSTRAINT osm_country_point_rank_constraint CHECK("rank" BETWEEN 1 AND 6);
CREATE INDEX IF NOT EXISTS osm_country_point_rank_idx ON __use_schema__.osm_country_point("rank");


-- Layer place - ./update_island_polygon.sql

-- etldoc:  osm_island_polygon ->  osm_island_polygon
CREATE OR REPLACE FUNCTION __use_schema__.update_osm_island_polygon() RETURNS VOID AS $$
BEGIN
  UPDATE __use_schema__.osm_island_polygon
  ...
  ANALYZE __use_schema__.osm_island_polygon;
END;
$$ LANGUAGE plpgsql;

SELECT __use_schema__.update_osm_island_polygon();


-- Layer place - ./update_island_point.sql

-- etldoc:  osm_island_point ->  osm_island_point
CREATE OR REPLACE FUNCTION __use_schema__.update_osm_island_point() RETURNS VOID AS $$
BEGIN
  UPDATE __use_schema__.osm_island_point
  SET tags = update_tags(tags, geometry)
  WHERE COALESCE(tags->'name:latin', tags->'name:nonlatin', tags->'name_int') IS NULL;
...
SELECT __use_schema__.update_osm_island_point();


-- Layer place - ./update_state_point.sql

ALTER TABLE __use_schema__.osm_state_point DROP CONSTRAINT IF EXISTS osm_state_point_rank_constraint;

-- etldoc: ne_10m_admin_1_states_provinces   -> osm_state_point
-- etldoc: osm_state_point                       -> osm_state_point

CREATE OR REPLACE FUNCTION __use_schema__.update_osm_state_point() RETURNS VOID AS $$
BEGIN
  ...
  WITH important_state_point AS (
      ...
      FROM mapdata_prod.ne_10m_admin_1_states_provinces AS ne, __use_schema__.osm_state_point AS osm
      WHERE
  ...
  UPDATE __use_schema__.osm_state_point AS osm
  -- Normalize both scalerank and labelrank into a ranking system from 1 to 6.
  ...
  DELETE FROM __use_schema__.osm_state_point WHERE "rank" IS NULL;

  UPDATE __use_schema__.osm_state_point
  SET tags = update_tags(tags, geometry)
  WHERE COALESCE(tags->'name:latin', tags->'name:nonlatin', tags->'name_int') IS NULL;
...
SELECT __use_schema__.update_osm_state_point();

-- ALTER TABLE osm_state_point ADD CONSTRAINT osm_state_point_rank_constraint CHECK("rank" BETWEEN 1 AND 6);
CREATE INDEX IF NOT EXISTS osm_state_point_rank_idx ON __use_schema__.osm_state_point("rank");


-- Layer place - ./update_city_point.sql

CREATE OR REPLACE FUNCTION __use_schema__.update_osm_city_point() RETURNS VOID AS $$
BEGIN

  -- Clear  OSM key:rank ( https://github.com/openmaptiles/openmaptiles/issues/108 )
  -- etldoc: osm_city_point          -> osm_city_point
  UPDATE __use_schema__.osm_city_point AS osm  SET "rank" = NULL WHERE "rank" IS NOT NULL;
  ...
  WITH important_city_point AS (
      ...
      FROM mapdata_prod.ne_10m_populated_places AS ne, __use_schema__.osm_city_point AS osm
      WHERE
      (
  ...
  UPDATE __use_schema__.osm_city_point AS osm
  ...
  UPDATE __use_schema__.osm_city_point
  SET tags = update_tags(tags, geometry)
  WHERE COALESCE(tags->'name:latin', tags->'name:nonlatin', tags->'name_int') IS NULL;
...
SELECT __use_schema__.update_osm_city_point();

CREATE INDEX IF NOT EXISTS osm_city_point_rank_idx ON __use_schema__.osm_city_point("rank");
