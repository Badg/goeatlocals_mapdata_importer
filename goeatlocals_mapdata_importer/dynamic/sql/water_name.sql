...
BEGIN
  ...
  UPDATE :use_schema.osm_marine_point AS osm SET "rank" = NULL WHERE "rank" IS NOT NULL;
  ...
  WITH important_marine_point AS (
      ...
      FROM mapdata_prod.ne_10m_geography_marine_polys AS ne, :use_schema.osm_marine_point AS osm
  ...
  UPDATE :use_schema.osm_marine_point AS osm
  SET "rank" = scalerank
  FROM important_marine_point AS ne
  WHERE osm.osm_id = ne.osm_id;

  UPDATE :use_schema.osm_marine_point
...
CREATE INDEX IF NOT EXISTS osm_marine_point_rank_idx ON :use_schema.osm_marine_point("rank");


CREATE OR REPLACE VIEW :use_schema.osm_water_lakeline_view AS
	SELECT wp.osm_id,
		...
		is_intermittent
    FROM :use_schema.osm_water_polygon AS wp
    INNER JOIN :use_schema.lake_centerline ll ON wp.osm_id = ll.osm_id
...
CREATE TABLE IF NOT EXISTS :use_schema.osm_water_lakeline AS
SELECT * FROM :use_schema.osm_water_lakeline_view;
DO $$
    BEGIN
        ALTER TABLE :use_schema.osm_water_lakeline
    ...
    END;
$$;
CREATE INDEX IF NOT EXISTS osm_water_lakeline_geometry_idx ON :use_schema.osm_water_lakeline USING gist(geometry);

-- Handle updates

CREATE OR REPLACE VIEW :use_schema.osm_water_point_view AS
    SELECT
        ...
        wp.is_intermittent
    FROM :use_schema.osm_water_polygon AS wp
    LEFT JOIN :use_schema.lake_centerline ll ON wp.osm_id = ll.osm_id
    WHERE ll.osm_id IS NULL AND wp.name <> ''
;

-- etldoc:  osm_water_polygon ->  osm_water_point
-- etldoc:  lake_centerline ->  osm_water_point
CREATE TABLE IF NOT EXISTS :use_schema.osm_water_point AS
SELECT * FROM :use_schema.osm_water_point_view;
DO $$
    BEGIN
        ALTER TABLE :use_schema.osm_water_point
    ...
    END;
$$;
CREATE INDEX IF NOT EXISTS osm_water_point_geometry_idx ON :use_schema.osm_water_point USING gist (geometry);
