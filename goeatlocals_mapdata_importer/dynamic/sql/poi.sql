...
DROP MATERIALIZED VIEW IF EXISTS :use_schema.osm_poi_stop_centroid CASCADE;
CREATE MATERIALIZED VIEW :use_schema.osm_poi_stop_centroid AS (
  SELECT
      uic_ref,
      count(*) as count,
      CASE WHEN count(*) > 2 THEN ST_Centroid(ST_UNION(geometry)) END AS centroid
  FROM :use_schema.osm_poi_point
	WHERE
...
DROP MATERIALIZED VIEW IF EXISTS :use_schema.osm_poi_stop_rank CASCADE;
CREATE MATERIALIZED VIEW :use_schema.osm_poi_stop_rank AS (
	SELECT
		p.osm_id,
-- 		p.uic_ref,
-- 		p.subclass,
	...
	FROM :use_schema.osm_poi_point p
		INNER JOIN :use_schema.osm_poi_stop_centroid c ON (p.uic_ref = c.uic_ref)
	WHERE
...
BEGIN
    UPDATE :use_schema.osm_poi_polygon
    SET geometry = normalize_poi_polygon_geo(geometry);

    UPDATE :use_schema.osm_poi_point
    ...
    ANALYZE :use_schema.osm_poi_polygon;
END;


BEGIN
    ALTER TABLE :use_schema.osm_poi_point
    ...
    UPDATE :use_schema.osm_poi_point p
    SET agg_stop = normalize_osm_poi_point_agg(p.subclass, r.rk)
    FROM :use_schema.osm_poi_stop_rank r
